...
function handleImageChange(e) {
    ...
    try {
        imageCompression(file, {
            ...
            if (preview) preview.src = URL.createObjectURL(compressedImageFile);
        });
    } catch (error) {
        console.error(error);
        compressedFiles.delete(e.target.name);
...
function addDetail() {
    ...
    detailItem.querySelector("input[name='details[0][kind]']").name =
        `details[${detailIndex}][kind]`;
    detailItem.querySelector("input[name='details[0][description]']").name =
        `details[${detailIndex}][description]`;
    detailItem.querySelector("input[name='details[0][price]']").name =
        `details[${detailIndex}][price]`;
    detailItem.querySelector("input[name='details[0][cost_price]']").name =
        `details[${detailIndex}][cost_price]`;
    detailItem.querySelector("input[name='details[0][image]']").name =
        `details[${detailIndex}][image]`;

    detailItem.querySelector("#details-0-kind").id =
        `details-${detailIndex}-kind`;
    detailItem.querySelector("#details-0-description").id =
        `details-${detailIndex}-description`;
    detailItem.querySelector("#details-0-price").id =
        `details-${detailIndex}-price`;
    detailItem.querySelector("#details-0-cost_price").id =
        `details-${detailIndex}-cost_price`;
    detailItem.querySelector("#details-0-image").id =
        `details-${detailIndex}-image`;
...
// Attach preview handlers to already-rendered details (edit page)
detailsContainer
    ?.querySelectorAll('input[name$="[image]"]')
    .forEach((input) => input.addEventListener("change", handleImageChange));
